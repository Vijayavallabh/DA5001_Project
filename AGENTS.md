...
**Status (2026-09-06, end of phase 2): both phases complete; nothing is running.** Phase 1: feat-001..009 and 013..015 done. Phase 2: feat-017..027 done (`GOAL.md` → Phase 2; plan v2 Sections 2–7); feat-012 is superseded by feat-024; feat-010/011 remain optional and unstarted. Human decisions D1/D2 were answered on 2026-09-06 (70B mirror downloaded; GPUs 1+2 for the 70B, shared 0/4 allowed for 8B jobs). The manuscript is final — 12 body pages, 18 with references and appendix, 0 overfull, 0 `??`, 125 cited entries of 128 — and artifact v2 is built (`artifact/`, 179 files + manifest; `artifact.zip` 23 MB). Only the human-only `feat-016` submission steps remain. Any further work is polish: re-verify with `./init.sh` and the manuscript command below before and after touching anything.
...
4. Read `feature_list.json` and `progress.md`; pick the lowest-numbered feature whose dependencies are `done` and whose status is `not-started` or `in-progress`. As of 2026-09-06 no such feature is left except the optional feat-010/011: default to verifying rather than adding scope, and never start feat-016 (human-only).
5. `git log --oneline -5`.
