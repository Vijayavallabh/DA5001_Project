...
The arm's numbers reach the manuscript through one appendix paragraph and one clause in the main
text's cost subsection (Section 2 until v9; Section 4.5, `sections/experiments.tex`, since the v10
restructure of 2026-09-24), and the committed statistic -- the paired difference -- was formed by hand at scoring time and lived
...
# The main-text clause lived in Section 2 (selection.tex) through v9; v10 moved it, unchanged in
# substance, to the cost subsection of Section 4 (sec:cost). The guard follows the claim.
MAIN = "sections/experiments.tex"


def test_section_2_and_the_appendix_agree_on_the_bigger_anchors_own_gain():
    """The main text states the gain in 3dp and the appendix in 4dp, which is how they drift."""
    g = float(rows()["G_A"]["value"])
    sec = open(tex(MAIN), encoding="utf-8").read().replace("\n", " ")
...
def test_section_2_points_at_the_appendix_that_answers_it():
    ...
    assert "bigger anchor" in para.lower() or "larger safe model" in para.lower(), \
        (m.group(1), "the main text's reference lands on a paragraph that is not about this claim")
