...
- Next: when `output/phase2/kl_sweep_hi/h1_summary.json` exists, rerun `analysis/pathwise_price.py --kl output/sweep_plain output/phase2/kl_sweep_hi --pathwise output/phase2/pathwise_sweep output/phase2/pathwise_sweep_hi --out results`, refresh the same-sweep KL numbers in the price paragraph of `sec:pathwise` (currently k ≤ 1 same-sweep, k ≥ 3 pathwise-only), recompile (body must still end on page 12), rebuild the artifact, commit, and close feat-026/027. Then the human's steps (feat-016): abstract registration Sep 22 (abstract is final), paper Sep 29, artifact repo Oct 2.
