...
- Goal: phase 1 (ship the SaTML 2027 submission) is COMPLETE (2026-09-05; commit 7b6961e). **Phase 2 in execution since 2026-09-06 00:45** under `~/sub/satml/IMPROVEMENT_PLAN.md` v2 (feat-017..027; `GOAL.md` Phase 2). The user answered D1 = ungated mirror `unsloth/Meta-Llama-3.1-70B` (downloaded, `hf_cache/`), D2 = 70B on GPUs 1+2, 8B jobs on shared GPUs 0/4 (PCI order; set `CUDA_DEVICE_ORDER=PCI_BUS_ID`).
- Done today: feat-019 code + tests + smoke (pathwise decoder, 0 violations), feat-022 (`results/warped_anchor.csv`), feat-023 (`results/budget_path.csv`, figure), burst audit (`results/burst_audit.csv`), new analysis scripts for feat-019/020/021/024, theory appendix (`sections/appendix_theory.tex`, Propositions 1–4), Related Work v2, Section VII skeleton with the warped-anchor subsection, budget-path paragraph in the attack results; `references.bib` 128 entries.
- Running in the background when this was written (check `output/phase2/*/run.log`): GPU 0 → `comp8b_kl` (KL composition, per-query logs) then `pathwise_sweep`; GPU 4 → `comp8b_pathwise`, `kl_sweep_conc`, `prefix_ablation`, `retries_kl`, `retries_pathwise`; GPUs 1+2 → `nm_smoke` (70B, 50 Harry Potter passages, k=−1, authors' decoding). The 70B smoke was slow (≈4 tokens/s at batch 4); suspected CPU offload of a few tensors under the 72/64 GiB caps: raise `--max-memory` to about `0=75GiB,1=70GiB`, confirm the new `[INFO] risky model device map` line shows no `cpu`, use `--batch-size 16`, then `scripts/run_natural_memorisation.sh 16` (feat-018).
- Next analyses once logs exist: `analysis/odometer.py --queries output/phase2/comp8b_kl/queries.jsonl` (feat-021), `analysis/concentration.py --logs output/phase2/kl_sweep_conc` (feat-020), `analysis/pathwise_price.py --kl output/sweep_plain --pathwise output/phase2/pathwise_sweep` and `analysis/extraction_cost.py --queries output/phase2/retries_*/queries.jsonl` (feat-019), `analysis/composition_attack.py --plot-only` per run, `analysis/latent_leakage.py` (feat-024, needs internet for Gutenberg), feat-025 numbers from `output/phase2/prefix_ablation`.
- **For the human (feat-016, unchanged):** phase-1 PDF preserved as `sub/satml/satml_2027_phase1_2026-09-06.pdf`; the working PDF is being restructured (currently 16 pages with forward references) and is not submittable until feat-026 completes; abstract registration Sep 22 with the v2 abstract.
- Branch / commit: `master`; latest work: phase-2 checkpoint (this commit).
## Resolved Decision
