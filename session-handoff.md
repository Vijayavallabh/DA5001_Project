...
- Goal: phase 1 COMPLETE (2026-09-05, commit 7b6961e). **Phase 2 experiments COMPLETE 2026-09-06 11:10** (feat-017..025 done, commit a3cf03c) under `~/sub/satml/IMPROVEMENT_PLAN.md` v2; **feat-026 (manuscript v2) and feat-027 (figures + artifact v2) remain.** Human decisions taken 2026-09-06: D1 = `unsloth/Meta-Llama-3.1-70B` mirror (cached in `hf_cache/`), D2 = 70B on GPUs 1+2, 8B jobs on shared GPUs 0/4 (PCI order).
...
- Manuscript (`/mnt/md0/IITM/BackUp/Home/vijayavallabh/sub/satml/satml_2027.tex`): all sections written including `sections/natural.tex`, `prefixdebt.tex`, `certificates.tex` (pathwise, concentration, warped, leakage, odometer + bank cap), `appendix_theory.tex`, Related Work v2; compiles with tectonic, 0 overfull, 0 ??, **21 pages uncompressed** (body must be ≤ 12). Phase-1 PDF kept as `satml_2027_phase1_2026-09-06.pdf`. Bibliography: 128 entries, 125 cited, all verified (`bibcheck_2026-09-06/`).
- Still running when this was written: `output/phase2/kl_sweep_hi` (GPU 1; KL k=3,5,10,20 on the 900 sweep prompts, to make the pathwise price same-sweep at every k → rerun `analysis/pathwise_price.py --kl output/sweep_plain output/phase2/kl_sweep_hi --pathwise output/phase2/pathwise_sweep output/phase2/pathwise_sweep_hi --out results` and refresh the price paragraph in `sec:pathwise`), and `output/phase2/retries_pathwise_lo` (GPU 2; k=2,3, 8 attempts → `analysis/extraction_cost.py --queries output/phase2/retries_pathwise_lo/queries.jsonl --constraint pathwise --limit 50 --out results --prefix extraction_cost_pathwise_lo`, then extend the cost paragraph).
- Next: feat-026 — compress to ≤ 12 body pages (candidates: merge sweep into results, shorten Related Work, move tab:warped/tab:bankcap details to text, trim protocol), rewrite abstract/intro contributions (C1–C6 + natural memorisation, pathwise, concentration, accounting, prefix debt, certificate card), conclusion, Open Science, LLM usage (GPU-hour tally: 70B ≈ 2 GPUs × 3.5 h; 8B sweeps/attacks ≈ 30 GPU-h), humanizer/no-ai-slop pass, hallucinator on the final PDF; feat-027 — regenerate figures via `figures/make_figures.py --copy-to <sub/satml/figures>` (add natural_memorisation/budget_path), rebuild `artifact/` with `scripts/build_artifact.sh artifact`. Abstract frozen by Sep 21 for the human's Sep 22 registration.
