#!/usr/bin/env bash
# results/onset_prediction_kl3m_seed.md -- wait for all three classes, merge, score.
...
. scripts/gpu_env.sh
set -a; . ./.env; set +a
LOG=output/logs/kl3mseed_merge.log
MERGED=output/phase5/sel_kl3m17b_64_seed52
...
for C in neutral creative factual; do
  D=output/phase5/sel_kl3m17b_64_seed52_$C
...
mkdir -p "$MERGED"
for C in neutral creative factual; do
  cp "output/phase5/sel_kl3m17b_64_seed52_$C/trajectories_k0_$C.jsonl" "$MERGED/" || exit 1
done
wc -l "$MERGED"/*.jsonl >> "$LOG"

echo "[merge] $(date +%H:%M:%S) START scoring, max-n 64, tag _kl3m17bseed52" >> "$LOG"
env $E .venv/bin/python analysis/selection_scaling.py --gen-dir "$MERGED" \
  --max-n 64 --reward-cache results/selection_rewards64_kl3m17bseed52.csv \
  --tag _kl3m17bseed52 --out results >> "$LOG" 2>&1
echo "[merge] $(date +%H:%M:%S) scoring rc=$? -- SEED ARM DRAINED" >> "$LOG"
