...
#   bash scripts/run_feat195.sh gpu4 | gpu12 | judge_chat | judge_he
set -u
cd "$(dirname "$0")/.." || exit 1
...
case "$1" in
  ...
  gpu12)
    ...
    run t07_70b $PY h1.py --k-values -1 0.5 1 20 $HE --risky-model-path unsloth/Meta-Llama-3.1-70B \
      --parallelize --trajectories-per-prompt 1 $CAPS --output-dir output/feat195/t07_70b ;;
  judge_chat)   # GPU 4, beside the pool; the judge is 3.8B
    export CUDA_VISIBLE_DEVICES=4
    wait_for 14400 chat || exit 1
  ...
  judge_he)     # after the pool is scored and the 70B is done; J1/J2 on GPU 1, J3/J4 on GPU 2
    ...
    wait ;;
  *) echo "usage: $0 gpu4|gpu12|judge_chat|judge_he"; exit 2 ;;
esac
