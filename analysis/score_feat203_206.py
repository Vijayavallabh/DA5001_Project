...
def score_205(res, outdir):
    ...
    # Post hoc, descriptive (appended after every registered row): the shares pooled over bytes, which is how
    # results/anchoredbyte.csv computes the temperature-1.0 figures the paper quotes (a mean of per-record shares
    # counts an empty record as 0 and is a different denominator, caution (ai)); empty records; and the prompts on
    # which the meter and its control serve the same text and are judged differently (each arm is judged in its own
    # batches, so identical inputs can differ by batch composition alone).
    txt = {}
    for k in ("0.1", "1e-06"):
        recs = [json.loads(line) for f in sorted(glob.glob(os.path.join(outdir, f"trajectories_k{k}_*.jsonl"))) for line in open(f)]
        nb = sum(r["aggregate"]["bytes_generated"] for r in recs)
        rows.append(row("posthoc", f"k={k}: binding share pooled over bytes (results/anchoredbyte.csv's construction)",
                        round(sum(r["aggregate"]["steps_binding"] for r in recs) / nb, 4), n=len(recs)))
        rows.append(row("posthoc", f"k={k}: forced share pooled over bytes",
                        round(sum(r["aggregate"]["steps_forced_safe"] for r in recs) / nb, 4), n=len(recs)))
        rows.append(row("posthoc", f"k={k}: records with no generated byte",
                        sum(r["aggregate"]["bytes_generated"] == 0 for r in recs), n=len(recs)))
        txt[k] = {r["metadata"]["prompt_id"]: r["aggregate"]["full_text"] for r in recs}
    pp = {r["prompt_id"]: r for r in csv.DictReader(open(os.path.join(res, "order_averaged_h2h_per_prompt_ab07_k01.csv")))}
    same = [p for p in txt["0.1"] if txt["0.1"][p] == txt["1e-06"][p]]
    flips = sum(pp[p]["u_metered_k0.1"] != pp[p]["u_anchor_k0"] for p in same)
    rows.append(row("posthoc", "prompts where the meter and its control serve identical text, judged differently", flips,
                    n=len(same)))
