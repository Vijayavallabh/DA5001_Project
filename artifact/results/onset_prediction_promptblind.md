...
### Scored 2026-09-26 17:15 (`.venv/bin/python analysis/score_feat217.py` -> `results/promptblind_scoring.csv`)

Scores: `analysis/promptblind_scorer.py`, eight shards on host B's H100s (32,000 draws, 500 prompts). Judge B on the
local A100s (comparators carried from the committed pass), judge G on host B's H100s (comparators carried from
feat-216's H100 pass). **Gates**: G0 PASS (full cache; no prompt has 64 empty draws, and `sel_pb` serves no empty
text); G2 PASS under both judges.

| | judge B | judge G |
|---|---|---|
| `gain(sel_pb)` over one draw | `+0.0405 [+0.0165, +0.0645]` RETAINS | `+0.012 [-0.023, +0.0475]` NONE |
| `DQ = sel_n64 - sel_pb` | `+0.061 [+0.0375, +0.084]` CONFIRMED | `+0.164 [+0.130, +0.198]` CONFIRMED |

**Prediction WRONG** in its first half: under judge G the vetted scorer keeps no resolvable gain (`DQ` CONFIRMED as
predicted). The unvetted Qwen scorer gains `+0.1015` (B) and `+0.176` (G) in the same passes. So removing the query
cap with this scorer costs most of the gain, and a vetted scorer that keeps it is open. Manuscript: Appendix
`app:promptblind` (appendix only; the 9-page body is unchanged).
