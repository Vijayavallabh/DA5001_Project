...
## Scoring, 2026-09-16 --- INVALID and ABANDONED. There is no band between converging and destroying.

```
bash scripts/run_convergence_reverse_probe.sh <lr> <gpu>     # 13:09-14:02, GPUs 0/1/4
bash scripts/run_entry_gate_probe.sh output/phase5/memrev_lr7E4 \
     output/phase5/anchor_kl3m-002-520m lr7E4 2              # 15:56-16:00, GPU 2
```

The interval is now sampled at five rates, and it has no interior:

| rate | epochs | final loss | stop-loss fires? | sampled `k=-1` |
|---|---|---|---|---|
| `6e-4` | 25/40 | `0.0189` | **yes** | `0.855` |
| **`7e-4`** | 40/40 | `4.2489` | no | **`0.0000`** (measured, `lcs_word` `1.52`) |
| `8e-4` | 40/40 | `3.9586` | no | not measured |
| `9e-4` | — | — | — | run killed at epoch 12 by another user's job taking the card |
| `1e-3` | 40/40 | `4.4826` | no | `0.0000` (measured, `lcs_word` `1.64`) |

The committed rule carries the smallest non-converging rate, **`7e-4`**; its measured gate is
`0.0000`, so **this arm is INVALID and, per the condition written down before it ran, ABANDONED. No
third attempt.** The lost `9e-4` run cannot change the selection, which depends only on the ordering
`7e-4 < 8e-4 < 9e-4`.

### The finding the abandonment represents, which the pre-registration named in advance

**On this pair the optimiser goes from converging comfortably to destroying the model across a 17%
change in learning rate, with nothing usable in between.** At `6e-4` the stop-loss fires at epoch 25
and the memoriser reproduces `0.855` of a passage. At `7e-4` the final model reproduces `0.0000`,
with `lcs_word` `1.52` --- *less* than the `1.73` an anchor with no exposure to the corpus manages on
the same passages.

`7e-4` is not monotonically destroyed, which is the part worth recording: its loss runs `2.48`,
`0.87`, `3.62`, `3.26`, `0.95`, `0.18`, **`0.137` at epoch 31**, `4.96` at 38, `4.25` at the cap. It
reaches a perfectly usable memoriser and then collapses, more than once. Whether a run at this rate
yields a memoriser or a wreck is decided by where the epoch cap happens to fall. That sharpens the
note already in AGENTS.md --- this recipe oscillates near convergence, so read `final_loss` and never
the curve --- from a caution about reading into a statement about the recipe itself.

### What the crossover was, and what it is now

Neither direction could be built:

- **Forward** (`onset_prediction_convergence_causal_60.md`): the non-converged cell cannot be made to
  converge. Its loss floors at `0.0222` --- two rates, two trajectories, the same value flat for
  three epochs --- and the recipe's stop-loss of `0.02` sits under that floor.
- **Reverse** (this arm): the converged cell cannot be made merely non-converged. It goes straight
  from converging to destroyed.

So **the convergence reading in the paper stays a correlation and is explicitly not promoted to a
cause.** That is the honest outcome of having tried, and it is stated in
`appendix_robustness.tex` and `appendix_limitations.tex` rather than left as an unreported failure.

What the attempts did establish, and what the paper now says instead, is narrower and better
founded: *"the stop-loss never fires"* is not one phenomenon. On Pleias-1.2B/BookMIA it means the
threshold lies below what the pair can reach; on KL3M-520M it can be induced only by destroying the
model. Two cells, two different reasons, one phrase that had been covering both.

**Cost of the crossover:** twelve fine-tunes and three entry-gate probes, about 9.5 GPU-hours, no
sweeps and no onset ratio computed at any point.
