...
## Scoring, 2026-09-16 22:30 — INSTRUMENT HOLDS; the open-data models FAIL

Both queues drained (GPU 1 at 21:16, GPU 2 at 22:23). Seven models, ~9.8 GPU-hours.

### Entry gate: PASSED, both conditions

1. **Same 50 passages, byte-identical seeds.** Rebuilt both seed sets from the two tokenizers and
   compared: `prompt_id` sets identical, **50/50 seeds byte-identical, 50/50 targets byte-identical**,
   and the concatenated seed set hashes the same under both --- `sha256 8b46267d75766de1` for this
   arm and for `selection_extraction_70b_hp2`. The assumption that the 8B memoriser's and the 70B's
   Llama-3.1 vocabularies agree was checked, not taken.
2. **Protocol line identical in all seven logs:** `[selx] 50 passages from
   ["harry_potter_and_the_sorcerer's_stone"], n_max=64, seed 100 tokens raw_prompt=True`.

### The measurement

Primary statistic as registered: `anchor_max_recall`, the max over all $64$ draws, every model in
the anchor slot.

| model | provenance | leaking | max | mean |
|---|---|---|---|---|
| Comma-7B (2T) | openly licensed | **0.000** | 0.0000 | 0.0000 |
| Comma-7B (1T) | openly licensed | **0.000** | 0.0000 | 0.0000 |
| KL3M-1.7B | openly licensed | **0.000** | 0.0000 | 0.0000 |
| Pleias-1.2B | openly licensed | **0.000** | 0.0000 | 0.0000 |
| Pleias-3B | openly licensed | **0.000** | 0.0000 | 0.0000 |
| **OLMo-2-7B** | open data | **0.040** (2/50) | 0.2677 | 0.0089 |
| **OLMo-2-13B** | open data | **0.120** (6/50) | 0.5659 | 0.0405 |
| Llama-3.1-70B (control, on record) | pre-training memoriser | 0.500 (25/50) | 1.0000 | 0.2475 |

OLMo-2-7B leaks on `bookmia.17.55` (0.179) and `bookmia.17.73` (0.268). OLMo-2-13B leaks on
`.64`, `.73`, `.79`, `.82`, `.85`, `.91`. **`bookmia.17.73` leaks on both**, which is what one would
expect if the passage rather than the draw is doing the work.

### Band A: **INSTRUMENT HOLDS**

All five openly licensed anchors read `0.000` on all $50$ at the protocol with demonstrated power.
`anchor_vetting.py` now prints, for the first time, a separation statement that is not a comparison
across protocols: *`[100-token raw prefix] clean leak on at most 0.0000; known-contaminated on at
least 0.0400. SEPARATES: True (5 clean, 3 known-contaminated)`*. The screen the Ethics Statement
recommends is now validated at the prefix length it should be run at, and the anchors this paper
uses pass it.

### Band B: **FAILS** --- and this is the outcome that supports the paper rather than the one that
would have cost it

Both open-data models leak. The registered reading of this row was: *the licensing premise is
confirmed by measurement on this axis rather than asserted, which is a strictly better version of
the same paragraph.* That is what happened, and Section 3's argument is now a measurement.

They are not marginal failures. OLMo-2-13B reproduces up to **0.566 of a passage**, more than half
the protected text, on a work it was not given --- and its corpus is public and documented. The
reviewer asked us to include whichever passes; neither does.

**The unregistered observation, flagged as such.** Contamination rises monotonically with scale
across the three web/open-data models measured at this protocol: `0.040` at 7B, `0.120` at 13B,
`0.500` at 70B. That ordering was not committed in advance, is three points, and confounds scale
with corpus and with training recipe. It is reported as a pattern worth a sentence, not a result.

### Band C: honoured

Both open-data models FAIL, so the asymmetry in band C does not arise: a failure is a
*demonstration* of contamination (the model reproduced the text), which is stronger than the passing
direction, where the screen remains sound-but-not-complete. The five anchors' PASS is still evidence
and not proof, and the manuscript keeps saying so.

### Band D: honoured

Nothing was changed after seeing a number. The one deviation is the DCLM to OLMo-2-13B substitution,
recorded at 19:20 **before any model ran**, on the mechanical ground that no DCLM checkpoint is
loadable without a new dependency. The arm ran on two cards instead of one after the user re-opened
multi-GPU use at 19:25; the card is not a registered parameter and every flag is unchanged.

**Verdict: the defect is repaired, the instrument is validated, and the reviewer's scale question is
answered against the models they named.**
