...
\section{Why a per-token budget is vacuous or trivial}\label{sec:frontier}
...
one from spreading it, which is what a rate does by construction, and that is the gap
Section~\ref{sec:selection} walks through. How close a sparse causal policy comes to
Theorem~\ref{thm:nfl}'s frontier stays open; Section~\ref{sec:orders} closes it empirically for the
deployed rule, not for the class.
...
and no budget in the family can say so (Appendix~\ref{app:opening}). Over ten safe models the margin
$s(x)/c_{\mathrm{use}}$ rises monotonically with capability (Appendix~\ref{app:scaling}): the raw
material is improving, and a per-token meter cannot use it.
