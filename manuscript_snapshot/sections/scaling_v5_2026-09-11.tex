...
% SUPERSEDED 2026-09-11 (v6): the table and both controls moved into sections/appendix_robustness.tex
% under \label{app:scaling}; Figure 1 and one paragraph of sections/frontier.tex carry the result in
% the main text. Kept verbatim for provenance; not \input by iclr_2027.tex.
...
ordinary instruction prompts. Rates are nats per character throughout: total string log-probability
is tokenizer-invariant and these models carry $64$k, $128$k and $152$k vocabularies, so no shared
...
the smallest. Both readings belong together: in absolute terms the range of budgets over which the certificate
says anything shrinks, so a deployer who fixed $k$ in nats finds it going vacuous under a better
anchor; in relative terms the two rates separate, and only the second reading is available to a
deployer who does not know the protected works in advance.
...
smaller, measured across six prompt classes and two budgets (Appendix~\ref{app:limitations}). Cheap-step selection does not account for that gap: the decoder pays more per unaltered step
than the cheapest matching fraction of the unconstrained distribution would, because it walks a
different trajectory, and its price is a property of that trajectory --- the approximation gap
