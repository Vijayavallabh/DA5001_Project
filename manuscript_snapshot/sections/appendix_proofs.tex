...
for every work, with equality iff the running ratio in Eq.~\eqref{eq:rates} is maximised at the last
token --- which a constant surprisal rate achieves only when $\delta = 0$.
\end{proposition}
...
it, and the metered decoder's best arm $7{,}995\times$. One report asks the same of the headline,
whether $\log 64$ is near $\Lambda^*_s$. On that pass, order-averaged and with each arm's own
