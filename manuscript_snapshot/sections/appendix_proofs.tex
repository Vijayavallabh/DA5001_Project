...
The threshold is where the tightest bound on $\Pr_q[E]$ reaches $1$, and it says nothing about how
fast that bound falls below it. There the orders differ by more than a constant. Write
$S = -\log p_s(E)$ and take $K < S$. At $\alpha = \infty$ the bound is $e^{K-S}$, and for
$\alpha \in (1,\infty)$ it is $e^{-(1-1/\alpha)(S-K)}$, both exponential in the shortfall $S-K$. At
$\alpha = 1$, the order the deployed decoder charges, it is only linear. Every $q$ with
$d(q(E) \,\|\, p_s(E)) \le K$ has $q(E) \le (K + \log 2)/S$, since $d(a \,\|\, e^{-S}) \ge aS - h(a)
\ge aS - \log 2$ with $h$ the binary entropy; and the law that emits $x$ with probability $a$ and
otherwise follows $p_s$ costs $aS - h(a) \le aS$ to first order in $e^{-S}$, so $a = K/S$ is within
budget. The tightest KL bound is therefore $K/S$ to within $\log 2/S$: a KL budget of half a work's
surprisal still permits the work half the time. With the window thresholds of
Table~\ref{tab:windowvacuity}: at $k = 0.5$, where neither of two judges separates the decoder from
its anchor, $K = 100$ nats against a $50$-token window's $159.8$ permits that window with probability
about $0.63$ under the KL charge and about $e^{-60}$ under the pathwise one. A KL certificate that
bounds the window by $\delta$ needs $K \le \delta S$, which by Proposition~\ref{prop:sparse} leaves
the decoder at most $\delta S/\varepsilon$ steps above $\varepsilon$ in expectation --- $1.6$ at
$\delta = 0.01$ and $\varepsilon = 1$ --- so at a finite horizon, and in the order that is deployed,
a certificate informative about a work is already a sparse one.
...
$E_w$ = ``reproduce some $w$-token window of the work'', and that analogue is sharper than the
whole-work statement in the direction that matters. A shorter event is less surprising under the
anchor, so $S_w(x) < S(x)$ and the certificate goes vacuous \emph{sooner}; quoting only the
whole-work figure is the lenient reading. It is also the wrong one here, because \textbf{every
extraction number in this paper is a window event} --- near-verbatim recall is scored on $50$-token
windows. The table below uses the surprisal of \emph{one} window, and the scored event is a union
over windows and positions; a union is at least as probable as any of its members --- in
particular as the window that opens the continuation --- so its surprisal is at most that window's,
and each threshold below is an \emph{upper} bound on where the certificate for the full event goes
vacuous. It can only go vacuous sooner.
...
window and the whole passage alike; the $50$-token event is vacuous at every $k \ge 0.8$, below the
smallest of them, and the authors' book setting is $k = 3$. Second, selection's certificate of $\log 64 = 4.159$ nats
...
Take $s(x)$ and $k_{\mathrm{crit}}(x)$ per \emph{token}, the unit the bucket accrues in: Eq.~\eqref{eq:rates}
with $N_t$ replaced by the token count $t$ and $N$ by $T$. Under the banking rule, if the decoder
emits each token of $x$ with probability at least $1-\eta$ then
$k \ge (1-\eta)\,k_{\mathrm{crit}}(x) - \log 2$, and $k_{\mathrm{crit}}(x) \ge s(x) + \delta/T$ for every
work, with equality iff the running ratio is maximised at the last token --- which a constant
surprisal rate achieves only when $\delta = 0$.
\end{proposition}
...
token, and $c_i \ge d(q \,\|\, e^{-s_i}) \ge (1-\eta)s_i - \log 2$ whenever $q \ge 1-\eta$. The banking
rule \citep[Algorithm~1]{he2026anchored} caps the charge on the first $t$ tokens at the allowance
accrued less the opening debt, $\sum_{i \le t} c_i \le tk - \delta$. Combining the two,
$tk \ge (1-\eta)\sum_{i \le t} s_i + \delta - t\log 2 \ge (1-\eta)\bigl(\sum_{i \le t} s_i + \delta\bigr) - t\log 2$,
and dividing by $t$ and maximising over $t$ gives the first claim. The second is the statement that
a running maximum dominates its own endpoint, $(S(x) + \delta)/T$. The per-character form of
Eq.~\eqref{eq:rates} is the same construction in tokenizer-free units, which is what compares
anchors with different vocabularies (Appendix~\ref{app:opening} reports both). \qed

\textbf{The equality condition is not a constant rate.} Equality holds exactly when the running ratio
$f(t) = (\sum_{i \le t} s_i + \delta)/t$ attains its maximum at the last step. At $\delta = 0$ a
constant rate does that, with $f \equiv s(x)$. At $\delta > 0$ it does the opposite: a constant rate
gives $f(t) = s(x) + \delta/t$, which is strictly \emph{decreasing}, so the maximum sits at $t=1$
and the inequality is strict. Equality under a positive opening debt needs the surprisal
back-loaded enough to outrun the decaying $\delta/t$ --- a work that begins predictably and ends
...
\paragraph{Selection anchoring does not evade the chain rule.}
It is tempting to argue that a selection rule is not a causal policy at all: every token it emits
was drawn from $p_{s,t}$, so $N_\varepsilon$ would be $0$ identically and its $\log n$ would be spent
where the chain rule does not reach. That is a statement about the \emph{sampler}, and nothing about
the served law follows from it. The served law $q$ is a distribution over
...
gains $+0.1045$ out of that budget, which is why the second horn is a shape and not a verdict.
...
\paragraph{Why no separation can be proved at the level of the served law.}\label{app:noseparation}
The theorem that would close the door Proposition~\ref{prop:sparse} leaves open --- no causal policy
with an $O(1)$ budget matches selection's utility --- is false as a statement about laws, and its
counterexamples are already in the paper. Every law on sequences is causal once factorised;
selection's served law is one, with pathwise budget $\log n$, and Theorem~\ref{thm:nfl}'s optimum,
the tilt $q^* \propto p_s e^{\lambda U}$, is another, with conditionals
$q^*_t(y_t \mid y_{<t}) = p_{s,t}(y_t)\, V_t(y_{\le t}) / V_{t-1}(y_{<t})$ for
$V_t = \mathbb{E}_{p_s}[e^{\lambda U} \mid y_{\le t}]$ and sequence divergence exactly
$\Lambda^*_s(\mathbb{E}_{q^*}[U])$. What separates a meter from both is \emph{information}: its kernel
at step $t$ is a function of $p_{s,t}$, $p_{r,t}$ and its bucket, while $V_t$ is an expectation over
continuations that neither next-token distribution contains. Selection estimates the tilt's effect
from $n$ finished rollouts and pays in draws rather than in divergence, and a causal policy that
estimated $V_t$ by rollouts would be selection spread over the decode loop. A provable separation is
therefore a statement about that information structure, and it is the one
Proposition~\ref{prop:imitation} makes: where the bucket is slack the local rule takes
$\theta_t = 1$ and pays the imitation cost whatever $U$ is.
...
Proposition~\ref{prop:sparse} is already non-asymptotic --- it holds at every $T$ --- but it bounds a mean, and the statement worth
...
$+0.0342$, but on the $73$ prompts that spend at all, with interval $[-0.0068,+0.0753]$, which includes zero and overlaps
...
it, and the metered decoder's best arm $7{,}995\times$. For the headline, the question is whether
$\log 64$ is near $\Lambda^*_s$. On that pass, order-averaged and with each arm's own
control as the safe law of $U$, selection at $n=64$ is $32\times$ the frontier on its $3.175$-nat
bound and the metered decoder $11{,}646\times$ on its $171.3$ nats spent
(\texttt{results/\allowbreak frontier\_\allowbreak distance\_\allowbreak h2h.csv}): nearer than at
$n=8$, and still more than an order of magnitude off it.
